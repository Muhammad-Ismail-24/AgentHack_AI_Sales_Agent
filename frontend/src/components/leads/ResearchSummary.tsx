  ...
  return (
    <section className="card overflow-hidden">
      <button
        ...
        className="flex w-full items-center justify-between px-5 py-4 text-left hover:bg-bark-700/40"
      >
      ...
      {isOpen && (
        <div className="animate-fade-in space-y-4 border-t border-bark-700 px-5 py-4">
          {facts.length > 0 && (
            <dl className="grid grid-cols-2 gap-x-6 gap-y-3">
              {facts.map((fact) => (
                <div key={fact.label}>
                  ...
                  <dd className="mt-0.5 truncate text-sm text-bark-200">
                    {fact.href ? (
                      <a
                        href={fact.href}
                        target="_blank"
          ...
          {lead.apollo_data && Object.keys(lead.apollo_data).length > 0 && (
            <details className="rounded-lg border border-bark-700 bg-bark-900/60 px-4 py-3">
              <summary className="cursor-pointer text-xs font-medium text-bark-400 hover:text-bark-200">
                Enrichment data
              </summary>
              <pre className="mt-3 max-h-64 overflow-auto whitespace-pre-wrap break-words text-xs text-bark-400">
